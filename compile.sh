...
if [ "$#" -eq 0 ]; then
    echo "Usage: ./compile.sh <file1.tony> [file2.tony ...]"
    exit 1
fi

echo "Preparing library (lib.c -> lib.ll)..."
clang -fno-builtin -S -emit-llvm lib.c -o lib.ll
if [ $? -ne 0 ]; then
...
for file in "$@"; do
    ...
    java -jar target/compiler-1.0.jar "$file" > /dev/null
    if [ $? -ne 0 ]; then
    ...
    # Link the generated code with lib.ll
